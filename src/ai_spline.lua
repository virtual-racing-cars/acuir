...
function splineMt:decode(reader)
        ...
        self.length = self.points[#self.points].length
                + (self.closed and self.points[1].pos:distance(self.points[#self.points].pos) or 0)
        self.hasSides = table.some(self.payloads, function(item) return item.sideLeft ~= 0 or item.sideRight ~= 0 end)
end
...
function splineMt:load(filename)
        try(function() self:decode(binaryUtils.readFile(filename)) end, function(err)
...
function splineMt:save(filename)
        try(
                function() self:encode():commit(filename):dispose() end,
                function(err) ac.error("Failed to save spline: %s" % err) end
        )
end
...
function splineMt:distanceBetween(pointIndex1, pointIndex2)
        ...
        if self.closed and d > self.length / 2 then d = self.length - d end
        return d
end

function splineMt:resize(size)
        if #self.points == size then return end
