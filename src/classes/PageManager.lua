...
function PageManager:registerPage(name, page) self.pages[name] = page end

function PageManager:setPage(name, skipUndo)
        if self.pages[name] then
                self.currentPage = self.pages[name]
                self.currentPageName = name
                if not skipUndo then self:makeUndo() end
        else
...
function PageManager:draw()
        if self.currentPage then return self.currentPage.draw(self) end
end

function PageManager:LoadStuff(name) self:setPage(name, true) end
